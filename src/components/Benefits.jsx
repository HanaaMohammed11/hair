...
import { GiHairStrands, GiScissors, GiHairBrush } from "react-icons/gi";
...
function Benefits() {
  ...
  const benefitsList2 = [
    { text: "علاج التقصف والفراغات", icon: <GiHairBrush /> },
      ...
      <div className="flex flex-col md:flex-row items-center justify-center gap-10 md:gap-20 flex-wrap md:flex-nowrap">
        ...
        <ul className="space-y-6 md:space-y-10 text-lg md:text-xl font-medium flex-1 animate-slideUp">
          {benefitsList1.map((item, idx) => (
            ...
            >
              <span className="text-2xl text-[#dba247] animate-fadeIn">
                {item.icon}
              </span>
              <span>{item.text}</span>
            </li>
